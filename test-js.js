...
-  [0, 2, 3, 0, 2, 3]; 
-  [1, 2, 0, 4, 5, 6];   
-  [1, 2, 0, 1, 2, 0];   "правильный отевет" // первый  аргумент -  чем заполняем, второй аргумент - с какой позиции заполняет, третий - по какую позицию заполняем.
-  [4, 5, 5, 1, 2, 3]; 
- 'никакой из перечисленных';
...
// --41--Что выведет в консоль код ниже
function Car(color) {
    var that = {};
    that.color = color;
    return that;
